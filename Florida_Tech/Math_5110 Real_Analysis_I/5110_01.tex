...
\end{align*}thus
\begin{align*}
	f'(x^n) &= \lim_{h\to 0} \frac{(x-h)^n-x^n}{h} \\
	&= \lim_{h\to 0} \frac{\sum_{k=0}^{n} \binom{n}{k} x^{n-k}(-h)^{k}-x^n}{h} 
\end{align*} remove the first entry from the summation
\begin{align*}
	f'(x^n) &= \lim_{h\to 0} \frac{x^n+\sum_{k=1}^{n} \binom{n}{k} x^{n-k}h^{k}-x^n}{h} \\
	&= \lim_{h\to 0} \frac{\sum_{k=1}^{n} \binom{n}{k} x^{n-k}h^{k}}{h} \\
	&= \sum_{k=1}^{n}\lim_{h\to 0} \frac{ \binom{n}{k} x^{n-k}h^{k}}{h} \\
	&= \lim_{h\to 0} \frac{ \binom{n}{1} x^{n-1}h}{h} + \lim_{h\to 0} \frac{ \binom{n}{2} x^{n-2}h^2}{h} + \cdots + \lim_{h\to 0} \frac{ \binom{n}{n} x^{0}h^n}{h}\\
	&= n x^{n-1} + \lim_{h\to 0} \frac{ \binom{n}{2} x^{n-2}h^2}{h} + \cdots + \lim_{h\to 0} \frac{ \binom{n}{n} x^{0}h^n}{h}
\end{align*}the $\LIM{h\to 0} \frac{h^k}{h} = \LIM{h \to 0} h^{k-1} = 0$ for all $k\ge 1$ therefore
\begin{align*}
	f'(x^n) &= nx^{n-1}
\end{align*}
}
...
\item $A = \{0,1,2,3,\dots\}$.  \BLUE{ $\sup A = \LIM{n\to \infty} n = \infty$, and $\inf A = 0$}
\end{itemize}

\item Find $\SUP{x \in (0, 1)}(1+x^2)^{-1}$

\BLUE{ Let $f(x) = (1+x^2)^{-1}$.  On the interval $(0,1)$ we can see that it is strictly decreasing ($f'(x) = -2(1+x^2)^{-2}(2x)< 0$ for all $x \in (0,1)$ ), that is $a<b \implies f(a)> f(b)$.  Thus, $\SUP{x \in (0, 1)}f(x) = f(0) = (1+0^2)^{-1} = 1$.}
...
\begin{description}
	...
	The square root is taken as a positive value.  $d_{l^2} ((x_1, \dots, x_n), (y_1, \dots, y_n))=0$ implies that each $x_i - y_i=0$ for all $i=1,\dots,n$ therefore $x=y$.
