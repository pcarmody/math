...
\end{align*}\BLUE{On the left, we integrate a single value over $X$ which is, basically, $Xc$ where $c = \liminf f_n$ over $X$.  On the right, we are taking the $\liminf$ of all areas under $f_n$, which, naturally, is larger or equal to the right.}
...
\end{align*}
    ...
    \item \textbf{Key inequalities}
        ...
        \begin{align * }
			\|fg\|_1 \le \|f\|_p\,\|g\|_q.
        \end{align * }  In addition, $p,q \in (1,\infty)$ and $f \in L^p(\mu)$ and $g \in L^q(\mu)$, then H\"olders Inequality becomes equality if and only if $|f|^p$ and $|g}^q$ are linearly dependent in $L^1(\mu)$, meaning that there exist real numbers $\alpha, \beta \ge 0$, not both of them zero, such that $\alpha|f|^p = \beta|g|^q$ a.e..
