...
\newcommand{\CLASSNAME}{Math 5050 -- Independent Study: Differential Geometry, Lie Groups}
...
\newpage
\textbf{\Large{Interesting tidbits}}

\begin{itemize}
	\item Interesting determinant identity.
	\begin{align*}
		\det e^M &= e^{\operatorname{tr}(M)}
	\end{align*}
	
	\item Complex numbers as two-by-two matrix
	\begin{align*}
		a + bi &\to \TWOXTWO{a}{-b}{b}{a}\\
		i &= \TWOXTWO{0}{-1}{1}{0} & \quad i^2 &= \TWOXTWO{-1}{0}{0}{-1} = -1\mathbb{I}_2
	\end{align*}
	\item Exponential Differential Operator is a translation.
	\begin{align*}
		e^{a\frac{d}{dx}}f(x) &= f(x+a)
	\end{align*}
	
	\item Local mapping from a tangent line to $S_1$ in contrast to a tangent space to $S_n$.
	\begin{align*}
		T_0(S_1) &\mapsto S_1 \\
			y &= e^{i\tan^{-1} y} \\
			\\
			T_0(S_n) &\mapsto S_n \\
				y &= e^{iM}
	\end{align*}where $M$ is an $n\times n$ matrix projecting onto $S_n$.
\end{itemize}
