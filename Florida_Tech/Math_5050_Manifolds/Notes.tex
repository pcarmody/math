...
\begin{enumerate}
	...
	\item \textbf{Wedge Product of $k$-form.} Recall: $dx^i\wedge dx^i =0$ for all $i=1,\dots,n$.  Therefore, $\wedge$ only makes sense to be defined on \textit{disjoint indice-lists}, that is, $I=\{i_1, \dots, i_k\}$ and $J=\{j_1, \dots, j_l\}$ such that $I\cap J =\emptyset$.  Then,
		...
		\begin{align*}
			...
			\tau(v) &= \sum_{J\in S_l} b_I(v) \bigwedge_{j\in J} dx^j & \text{where each } \bigwedge_{j\in J} dx^j \text{ is  basis vector for } \Omega^l \\
			(\omega \wedge \tau)(u,v) &= \PAREN{\sum_{I\in S_k} a_I(u) \bigwedge_{i\in I} dx^i}\wedge\PAREN{\sum_{J\in S_l} b_I(v) \bigwedge_{j\in J} dx^j} \\
			&= \sum_{I\in S_k,\,J\in S_l} a_I(u)b_I(v)\underbrace{\PAREN{ \bigwedge_{i\in I} dx^i \, \wedge \, \bigwedge_{j\in J} dx^j }}_{\text{basis vector of $(k+l)$-form}}
		\end{align*}
		}
