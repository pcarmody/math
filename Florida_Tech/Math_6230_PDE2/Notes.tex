...
\newpage
\subsection*{Motivation for H\"older Norm?}
...
\newpage
\subsection*{Exercises for mollification}
...
\item \textbf{Exercise 1: Approximation of Identity}
...
\item  \textbf{Exercise 2: Density of Smooth Functions in \( L^p \)}
...
\item  \textbf{Exercise 3: Mollification and Derivatives}
...
\item  \textbf{Exercise 4: A Product Rule for Sobolev Functions}
...
\item \textbf{Exercise 5: Friedrichs' Inequality}
...
\HLINE
\newpage
\subsection*{Exercises for Convolusions.}
...
\begin{description}
\item \textbf{Exercise 1: Basic Properties of Convolution}
...
\item \textbf{Exercise 2: Young's Inequality for Convolutions}
...
\item \textbf{Exercise 3: Convolution and the Fourier Transform}
...
\item  \textbf{Exercise 4: Solving a Differential Equation with Convolution}
...
\item  \textbf{Exercise 5: Support of a Convolution}
...
\newpage
\subsection*{Describe the $H^n(H)$ Space.}
...
\HLINE
\newpage
\subsection*{Define elliptical, hyperbolic and parabolic PDE}.
\item \textbf{Explain the terms elliptical, hyperbolic and parabolic in the context of PDE}

The classification into \DEFINE{elliptic, parabolic, and hyperbolic PDEs} is really about the \textit{geometry and behavior of solutions}, not just algebra. The cleanest way to see it is through the \DEFINE{second-order linear PDE}.

---
\begin{description}

\item \textbf{1. The general second-order PDE}

In two variables, consider:
\begin{align*}
...
\item \textbf{2. The three types}

\begin{description}
\item \textbf{(a) Elliptic: $B^2 - AC < 0$}
\begin{description}
\item \textit{Example:}
...
\begin{itemize}
\item No real characteristic directions
\item Behavior is \DEFINE{spatially smooth and global}
\item Disturbances spread instantly everywhere
\end{itemize}

\item \textit{Key properties:}

\begin{itemize}
\item Solutions are very smooth (analytic if coefficients are nice)
\item No propagation of waves
\item Boundary conditions determine everything
\end{itemize}

\item \textit{Physical intuition:}

\begin{itemize}
\item Steady-state equilibrium
\item Example: temperature in a solid at equilibrium
\end{itemize}

---
\end{description}

\item \textbf{(b) Hyperbolic: $B^2 - AC > 0$}
\begin{description}
\item \textit{Example:}

\begin{align*}
u_{tt} - c^2 u_{xx} = 0 \quad \text{(wave equation)}
\end{align*}

\item \textit{Meaning:}

\begin{itemize}
\item Two real characteristic directions
\item Information propagates along \DEFINE{characteristic curves}
\end{itemize}

\item \textit{Key properties:}

\begin{itemize}
\item Finite speed of propagation
\item Signals travel like waves
\item Sharp fronts and discontinuities can occur
\end{itemize}

\item \textit{Physical intuition:}

\begin{itemize}
\item Vibrations, waves, signals
\item Example: string vibration, sound waves
\end{itemize}
\end{description}

---

\item \textbf{(c) Parabolic: $B^2 - AC = 0$}

\begin{description}
\item \textit{Example:}
\begin{align*}
u_t - k u_{xx} = 0 \quad \text{(heat equation)}
\end{align*}

\item \textit{Meaning:}

\begin{itemize}
\item One repeated characteristic direction
\item Intermediate between elliptic and hyperbolic
\end{itemize}

\item \textit{Key properties:}

\begin{itemize}
\item Diffusion/smoothing over time
\item Infinite propagation speed (but decays rapidly)
\item Irreversible behavior (time direction matters)
\end{itemize}

\item \textit{Physical intuition:}

\begin{itemize}
\item Heat flow, diffusion
\item Information spreads but *dissipates*
\end{itemize}
\end{description}
...
\item \textbf{3. Geometric interpretation (very important)}

The classification comes from the quadratic form:
\begin{align*}
A,dx^2 + 2B,dx,dy + C,dy^2
\end{align*}

This behaves like conic sections:

\begin{tabular}{|l|l|l|}
\hline
   Type       & Geometry  & Analogy            \\   
\hline
   Elliptic   & Ellipse   & No real directions \\   
   Hyperbolic & Hyperbola & Two directions     \\   
   Parabolic  & Parabola  & One direction      \\   
\hline
\end{tabular}
...
\begin{itemize}
\item \DEFINE{Hyperbolic} → real characteristic curves (wave propagation)
\item \DEFINE{Elliptic} → no real characteristics (global coupling)
\item \DEFINE{Parabolic} → degenerate case
\end{itemize}

---

\item \textbf{4. Characteristics (the real heart of it)}
...
\begin{itemize}
\item Hyperbolic → two real solutions → two families of curves
\item Parabolic → one repeated solution
\item Elliptic → no real solutions
\end{itemize}

These curves are where:

\begin{itemize}
\item information travels (hyperbolic)
\item PDE reduces to ODEs
\end{itemize}

---

\item \textbf{5. Big-picture intuition}

\begin{description}
\item \DEFINE{Elliptic:}

\item \textit{“Everything depends on everything else at once.”}

\begin{itemize}
\item Global constraint
\item No notion of time evolution
\end{itemize}

---

\item \DEFINE{Hyperbolic:}

\item \textit{“Information travels along paths.”}

\begin{itemize}
\item Signals move with finite speed
\item Domain of dependence is finite
\end{itemize}

---

\item \DEFINE{Parabolic:}

\item \textit{“Information diffuses and forgets its past.”}

\begin{itemize}
\item Smooths out irregularities
\item Has a preferred time direction
\end{itemize}

\end{description}
---

\item \textbf{6. In higher dimensions}

For:
\begin{align*}
\sum_{i,j} a_{ij} u_{x_i x_j}
\end{align*}

Look at eigenvalues of matrix $A = (a_{ij})$:

\begin{itemize}
\item \DEFINE{Elliptic}: all eigenvalues same sign
\item \DEFINE{Hyperbolic}: mixed signs
\item \DEFINE{Parabolic}: at least one zero eigenvalue
\end{itemize}

---

\item \textbf{7. Why this matters}

This classification determines:

\begin{itemize}
\item What kind of boundary/initial conditions are appropriate
\item Whether solutions are smooth
\item How information propagates
\item Which methods work (Fourier, energy, maximum principle, etc.)
\end{itemize}

---
\end{description}

If you want, I can connect this directly to:

\begin{itemize}
\item characteristics and change of variables,
\item energy estimates,
\item or how this classification appears on manifolds (since you’ve been working in that direction).
\end{itemize}

\HLINE
\newpage
\subsection*{How do you derive a Green's Function?}

\item \textbf{How do you derive a Green's Function?}

Deriving a Green’s function is less about a single recipe and more about a \textit{strategy tailored to the operator and domain}. That said, there is a clear general framework you can follow.

---

\begin{description}
\item \textbf{1. What a Green’s function is}

Suppose you have a linear PDE operator $L$ and want to solve
\begin{align*}
L u(x) = f(x), \quad x \in \Omega
\end{align*}
with some boundary conditions.

The Green’s function $G(x,\xi$) is defined by
\begin{align*}
L_x G(x,\xi) = \delta(x-\xi)
\end{align*}
(with the same boundary conditions in $x$).

Once you have $G$, the solution is
\begin{align*}
u(x) = \int_\Omega G(x,\xi), f(\xi), d\xi.
\end{align*}

So the whole problem reduces to:
\DEFINE{solve the PDE with a point source}.

---

\item \textbf{2. General derivation strategy}

\begin{description}
\item \DEFINE{Step 1: Identify the operator and domain}

\begin{itemize}
\item Example: $L = -\Delta$, or $L = \partial_t - \Delta$
\item Domain: $\mathbb{R}^n$, bounded domain, interval, etc.
\item Boundary conditions matter *a lot*
\end{itemize}

---

\item \DEFINE{Step 2: Solve the fundamental solution (free space)}

First ignore boundaries and solve:
\begin{align*}
L G(x,\xi) = \delta(x-\xi), \quad x \in \mathbb{R}^n
\end{align*}
...
\item Symmetry (radial solutions)
\item ODE reduction (in 1D)
\end{itemize}

---

\item \DEFINE{Example: Laplacian in $\mathbb{R}^3$}
...
Using symmetry, assume $G = G(r$), $r = |x|$, then:
\begin{align*}
\Delta G = \frac{1}{r^2}\frac{d}{dr}\left(r^2 \frac{dG}{dr}\right)
\end{align*}
...
\item \DEFINE{Step 3: Impose boundary conditions}

If the domain is not all of $\mathbb{R}^n$, modify $G$ so that:

\begin{itemize}
\item $G(x,\xi$) satisfies boundary conditions in $x$
\end{itemize}

Methods:

\begin{itemize}
\item \DEFINE{Method of images}
\item \DEFINE{Eigenfunction expansions}
\item \DEFINE{Separation of variables}
\end{itemize}

---

\item \DEFINE{Example: Interval $[0,L]$}

For:
\begin{align*}
-u'' = f(x), \quad u(0)=u(L)=0
\end{align*}
...
\begin{itemize}
\item For $x<\xi$: $G = A x$
\item For $x>\xi$: $G = B (L-x)$
\end{itemize}

Apply:

\begin{itemize}
\item Continuity at $x=\xi$
\item Jump condition:
\begin{align*}
  G'( \xi^+) - G'(\xi^-) = -1
\end{align*}
...
Result:
\begin{align*}
G(x,\xi) = \BINDEF{ \frac{x(L-\xi)}{L}, & x<\xi }{ \frac{\xi(L-x)}{L}, & x>\xi s}
\end{align*}

---

\item \DEFINE{Step 4: Verify the delta property}

You must check:

\begin{itemize}
\item $G$ is continuous
\item Derivative has correct jump:
\begin{align*}
  \text{jump in derivative} = 1
\end{align*}
\end{itemize}
...
\item \DEFINE{Step 5: Build the solution}
...
\item \textbf{3. Key intuition}

A Green’s function is:

\DEFINE{The response of the system to a point source.}

So you can think:

\begin{itemize}
\item General forcing = superposition of point sources
\item Solution = sum (integral) of responses
\end{itemize}

---

\item \textbf{4. Common methods summarized}

\begin{description}
\item \DEFINE{1. Fourier transform (best for $\mathbb{R}^n$)}

Turn PDE into algebra:
\begin{align*}
\hat{G}(k) = \frac{1}{L(k)}
\end{align*}
...
\item \DEFINE{2. Eigenfunction expansion}

If $L\phi_n = \lambda_n \phi_n$, then:
\begin{align*}
G(x,\xi) = \sum_n \frac{\phi_n(x)\phi_n(\xi)}{\lambda_n}
\end{align*}

---

\item \DEFINE{3. Method of images}

Used for simple domains (half-space, etc.)

---

\item \DEFINE{4. Direct ODE approach (1D)}

Solve piecewise and enforce jump conditions.

---
\end{description}

\item \textbf{5. Subtle but important point}

Green’s functions are:

\begin{itemize}
\item \DEFINE{not unique} unless boundary conditions are fixed
\item depend heavily on domain geometry
\end{itemize}

---

\item \textbf{6. If you want to go deeper}

Given your background (you’ve been working with tensors and PDEs), the next level is:

\begin{itemize}
\item Green’s functions on manifolds
\item Heat kernel as a Green’s function
\item Connection with fundamental solutions and distributions
\end{itemize}
...
\begin{itemize}
\item Derive one fully step-by-step (Laplace, heat, wave), or
\item Show how Green’s functions relate to weak solutions and distributions.
\end{itemize}


\HLINE
\newpage
\subsection*{Explain Sobolev Embedding}

\item \textbf{Explain Sobolev Embedding}

The \DEFINE{Sobolev Embedding Theorem} tells us that functions with weak derivatives in $L^p$ automatically belong to better spaces—typically some $L^q$ space, or even a Hölder continuous space.

It is one of the most important tools in PDE and functional analysis because it converts \DEFINE{derivative information} into \DEFINE{regularity information}.
...
Suppose $u\in W^{1,p}(U$).
This means:

    \begin{itemize}
    \item $u\in L^p(U$)
    \item its first weak derivatives are also in $L^p(U$)
    \end{itemize}

Then Sobolev embedding says that $u$ often belongs to a larger space like:

    \begin{itemize}
    \item $L^q(U$) for some $q>p$,
    \item or $C^{0,\alpha}(U$),
    \item or even $L^\infty(U$),
    \end{itemize}

depending on the dimension $n$ and the exponent $p$.

---

\item \textbf{2. The standard $W^{1,p}\to L^q$ embedding}

Let $U\subset \mathbb{R}^n$ be bounded and regular enough.
...
p^*=\frac{np}{n-p}
\end{align*}
is called the \DEFINE{Sobolev conjugate exponent}.

This means there exists $C>0$ such that
\begin{align*}
|u|_{L^q(U)}\le C|u|_{W^{1,p}(U)}.
\end{align*}

---

\textbf{Example: $H^1(U)=W^{1,2}(U)$}

If $n\ge 3$, then
\begin{align*}
H^1(U)\hookrightarrow L^{2n/(n-2)}(U).
\end{align*}

That is,
\begin{align*}
|u|_{L^{2n/(n-2)}}\le C|u|_{H^1}.
\end{align*}

This is exactly what was used in your PDE problem.

---

\item \textbf{3. Why the exponent $p^*=\frac{np}{n-p}$?}

The exponent comes from balancing the integrability of $u$ and its derivative.

Roughly:

    \begin{itemize}
    \item differentiating costs one power of length,
    \item integration in dimension $n$ gives scaling by $n$,
    \item balancing these yields
  \begin{align*}
...
This is the largest $L^q$ space you can embed into in general.

---

\item \textbf{4. The three cases}

---
    \begin{description}
    \item \textbf{Case 1: $p<n$}

Then
\begin{align*}
W^{1,p}(U)\hookrightarrow L^q(U),\quad q\le p^*.
\end{align*}

This gives \DEFINE{better integrability}.

Example:
\begin{align*}
H^1(U)\hookrightarrow L^6(U)\quad \text{if }n=3.
\end{align*}

---

    \item \textbf{Case 2: $p=n$}

Then
\begin{align*}
W^{1,n}(U)\hookrightarrow L^q(U)\quad \text{for all }q<\infty,
\end{align*}
but generally not into $L^\infty$.

This is the borderline case.

---

    \item \textbf{Case 3: $p>n$}

Then
\begin{align*}
W^{1,p}(U)\hookrightarrow C^{0,\alpha}(\overline U),
\end{align*}
where
\begin{align*}
\alpha=1-\frac np.
...
This means functions are Hölder continuous.

So if derivatives are sufficiently integrable, the function is actually continuous.

---
    \end{description}

\item \textbf{5. The special case $n=2$}

For $H^1(U$) in dimension 2:

\begin{align*}
H^1(U)\hookrightarrow L^q(U)\quad \forall q<\infty.
\end{align*}

This was exactly the fact used in part $i$ of your boundary value problem.

Because $2$ is the critical dimension for $H^1$:

\begin{align*}
\frac{2n}{n-2}
\end{align*}
becomes infinite when $n=2$.

So in 2D, $H^1$ embeds into every finite $L^q$.

---

\item \textbf{6. Why this matters in PDE}

Sobolev embedding lets us estimate nonlinear terms.

Example:
\begin{align*}
\int_U c(x)u(x)v(x),dx.
\end{align*}

To control this with Hölder, we need $u,v\in L^q$.

Sobolev embedding gives:
\begin{align*}
u,v\in H_0^1(U)\implies u,v\in L^q(U),
\end{align*}
which makes the bilinear form bounded.

This is why Lax–Milgram works.

---

\item \textbf{7. Continuous embedding notation}
...
means:
	...
	\begin{enumerate}
    \item Every $u\in W^{1,p}(U$) lies in $L^q(U$),
    \item There is a constant $C$ such that
\begin{align*}
   |u|_{L^q}\le C|u|_{W^{1,p}}.
\end{align*}
	\end{enumerate}
...
\item \textbf{8. Compact embedding}

A stronger result is the \DEFINE{Rellich–Kondrachov theorem}:

If $q<p^*$, then
\begin{align*}
W^{1,p}(U)\hookrightarrow\hookrightarrow L^q(U),
\end{align*}
meaning the embedding is \DEFINE{compact}.

So bounded sequences in $W^{1,p}$ have convergent subsequences in $L^q$.

This is essential for nonlinear PDEs and variational methods.

---

\item \textbf{9. Intuition}

If the derivative of $u$ is in $L^p$, then $u$ cannot oscillate too wildly.

Greater derivative control gives greater regularity:

    \begin{itemize}
    \item weak derivative in $L^p$ $\Rightarrow$ better integrability,
    \item stronger $p$ $\Rightarrow$ continuity,
    \item even stronger $\Rightarrow$ differentiability.
    \end{itemize}

Sobolev embedding quantifies that.

---

\item \textbf{10. The two embeddings from your problem}

\begin{itemize}
\item For $n=2$:

\begin{align*}
H_0^1(U)\hookrightarrow L^q(U)\quad \forall q<\infty.
\end{align*}

\item For $n\ge 3$:

\begin{align*}
H_0^1(U)\hookrightarrow L^{2n/(n-2)}(U).
\end{align*}

These are the precise embeddings needed to estimate
\begin{align*}
\int_U c u v.
\end{align*}
...
If you'd like next, I can explain \DEFINE{why $H_0^1(U)\hookrightarrow L^{2n/(n-2)}(U)$} holds by proving the Sobolev inequality in the $H^1$ case.
