...
\BLUE($T \in \LL(\R^2,\PP_4(\F))$ and $T(a,b)=ax^3+bx^4$
}

\item Suppose $V$ is a vector space and $W,T, \in \LL(V,V)$ are such that 
\begin{align*}
...
\item Prove that there does not exist a linear map $T\,:\,\R^4\to\R^5$ such that 
\begin{align*}
	\RANGE T = \NULL T
\end{align*}

\item Suppose $V$ and $W$ are funite-dimensional with $2 \le \dim V \le \dim W$.  Show that $\{T\in ]LL(V,W)\,:\,T$ is not injectife $\}$ is not a suspace of $\LL(V,W)$

\item Suppose $V$ adn $W$ are finite-dimensional with $\dim V \ge\dim W\ge 2$.  Show that $\{T\in \LL(V,W)\,:\,T$ is not surjective $\}$ is not a subspace of $\L:L(V,W)$.
